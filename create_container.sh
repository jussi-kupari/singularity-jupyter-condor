...
usage() {
    ...
    echo ""
    echo "Examples:"
    echo "  $0 scanpy.def                      # Creates scanpy.sif"
    echo "  $0 scanpy.def my_container.sif     # Creates my_container.sif"
    echo ""
...
echo "=== Singularity Container Builder (Python + Scanpy) ==="
...
if singularity build --fakeroot "$CONTAINER_NAME" "$DEFINITION_FILE" 2>&1 | tee singularity_build.out; then
    ...
    if singularity exec "$CONTAINER_NAME" python --version >/dev/null 2>&1; then
        echo "✓ Python is available"
    else
        echo "✗ Python not available"
    fi

    echo "Testing Scanpy installation..."
    if singularity exec "$CONTAINER_NAME" python -c "import scanpy" >/dev/null 2>&1; then
        echo "✓ Scanpy loaded successfully"
    else
        echo "✗ Scanpy failed to load"
    fi

    echo "Testing Jupyter installation..."
    if singularity exec "$CONTAINER_NAME" jupyter --version >/dev/null 2>&1; then
